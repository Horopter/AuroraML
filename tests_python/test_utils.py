...
class TestUtils(unittest.TestCase):
    ...
    def test_multiclass_utilities(self):
        ...
        # Test is_multiclass
        is_multi = auroraml.utils.multiclass.is_multiclass(self.y_multiclass)
        self.assertTrue(is_multi)
        
        is_multi_binary = auroraml.utils.multiclass.is_multiclass(self.y)
        self.assertFalse(is_multi_binary)
        
        # Test unique_labels
        unique = auroraml.utils.multiclass.unique_labels(self.y_multiclass)
        self.assertGreater(len(unique), 2)
        
        # Test type_of_target
        target_type = auroraml.utils.multiclass.type_of_target(self.y)
        self.assertIn(target_type, ["binary", "multiclass", "continuous"])

    def test_resample(self):
        """Test resampling utilities"""
        # Test resample
        X_resampled, y_resampled = auroraml.utils.resample.resample(
        ...
        auroraml.utils.resample.shuffle(X_shuffled, y_shuffled, random_state=42)
    ...
    def test_validation(self):
        ...
        # Test check_finite
        is_finite = auroraml.utils.validation.check_finite(self.X)
        self.assertTrue(is_finite)
        
        # Test check_has_nan
        X_with_nan = self.X.copy()
        X_with_nan[0, 0] = np.nan
        has_nan = auroraml.utils.validation.check_has_nan(X_with_nan)
        self.assertTrue(has_nan)
        
        # Test check_has_inf
        X_with_inf = self.X.copy()
        X_with_inf[0, 0] = np.inf
        has_inf = auroraml.utils.validation.check_has_inf(X_with_inf)
        self.assertTrue(has_inf)

    def test_class_weight(self):
        """Test class weight utilities"""
        weights = auroraml.utils.class_weight.compute_class_weight("balanced", self.y)
        self.assertIsInstance(weights, dict)
        self.assertGreater(len(weights), 0)
        
        sample_weights = auroraml.utils.class_weight.compute_sample_weight(self.y, weights)
        self.assertIsInstance(sample_weights, dict)

    def test_array_utilities(self):
        """Test array utilities"""
        # Test issparse
        is_sparse = auroraml.utils.array.issparse(self.X)
        self.assertFalse(is_sparse)  # Eigen matrices are always dense
        
        # Test shape
        shape = auroraml.utils.array.shape(self.X)
