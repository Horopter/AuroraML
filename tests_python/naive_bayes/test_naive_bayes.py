...
class TestGaussianNB(unittest.TestCase):
    ...
    def test_predict_log_proba(self):
        ...
        # Calculate log probabilities from probabilities
        log_probabilities = np.log(probabilities + 1e-10)  # Add small epsilon to avoid log(0)
        
        self.assertEqual(log_probabilities.shape, (len(self.X_test), 2))
        
        # Convert back to probabilities and check consistency
        probabilities_from_log = np.exp(log_probabilities)
        self.assertTrue(np.allclose(np.sum(probabilities_from_log, axis=1), 1.0, atol=1e-5))
...
class TestGaussianNBIntegration(unittest.TestCase):
    ...
    def test_hyperparameter_tuning(self):
        ...
        self.assertIn('var_smoothing', best_params)
