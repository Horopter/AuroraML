...
public:
    SVR(double C = 1.0, double epsilon = 0.1, int max_iter = 1000, 
        ...
        const std::string& kernel = "linear", double gamma = 1.0,
        double degree = 3.0, double coef0 = 0.0);
 ...
 * Kernelized classifier using one-vs-rest strategy.
 */
class SVC : public Estimator, public Classifier {
private:
    bool fitted_ = false;
    MatrixXd X_train_;
    VectorXi classes_;
    MatrixXd alphas_; // columns correspond to classes
...
public:
    SVC(const std::string& kernel = "rbf", double C = 1.0, int max_iter = 1000,
        double lr = 0.01, int random_state = -1,
        double gamma = 1.0, double degree = 3.0, double coef0 = 0.0);
