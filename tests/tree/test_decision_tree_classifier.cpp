...
class DecisionTreeClassifierTest : public ::testing::Test {
protected:
    void SetUp() override {
...
// Positive test cases
TEST_F(DecisionTreeClassifierTest, DecisionTreeClassifierFit) {
    tree::DecisionTreeClassifier dt("gini", 5, 2, 1, 42);
    dt.fit(X, y_classification);
    
    EXPECT_TRUE(dt.is_fitted());
}

TEST_F(DecisionTreeClassifierTest, DecisionTreeClassifierPredictClasses) {
    tree::DecisionTreeClassifier dt("gini", 5, 2, 1, 42);
    dt.fit(X, y_classification);
    
    VectorXi y_pred = dt.predict_classes(X_test);
...
TEST_F(DecisionTreeClassifierTest, DecisionTreeClassifierPredictProba) {
    tree::DecisionTreeClassifier dt("gini", 5, 2, 1, 42);
    dt.fit(X, y_classification);
    
    MatrixXd y_proba = dt.predict_proba(X_test);
...
TEST_F(DecisionTreeClassifierTest, DecisionTreeClassifierPerformance) {
    tree::DecisionTreeClassifier dt("gini", 5, 2, 1, 42);
    dt.fit(X, y_classification);
...
TEST_F(DecisionTreeClassifierTest, DecisionTreeClassifierDifferentCriterion) {
    std::vector<std::string> criteria = {"gini", "entropy"};
    
    for (const std::string& criterion : criteria) {
        tree::DecisionTreeClassifier dt(criterion, 5, 2, 1, 42);
        dt.fit(X, y_classification);
        EXPECT_TRUE(dt.is_fitted());
...
// Negative test cases
TEST_F(DecisionTreeClassifierTest, DecisionTreeClassifierNotFitted) {
    tree::DecisionTreeClassifier dt("gini", 5);
...
TEST_F(DecisionTreeClassifierTest, DecisionTreeClassifierWrongFeatureCount) {
    tree::DecisionTreeClassifier dt("gini", 5, 2, 1, 42);
    dt.fit(X, y_classification);
...
TEST_F(DecisionTreeClassifierTest, DecisionTreeClassifierEmptyData) {
...
TEST_F(DecisionTreeClassifierTest, DecisionTreeClassifierDimensionMismatch) {
